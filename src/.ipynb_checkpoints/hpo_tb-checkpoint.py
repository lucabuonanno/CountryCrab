...
def optimize(scheduler_name: t.Optional[str] = None) -> None:
    ...
    # test only size 20
    #instance_list = [I_V_opt[200][0]]      
    search_space = {
        "noise": tune.uniform(0.4, 1.2),
        "instance": tune.grid_search(I_V_opt_flat),
        #"instance": tune.grid_search(instance_list),
    }

    # you can select how much of the gpu memory is used by any instance. This is a bit of a trial and error, start low and try to increase it until it works basically
    ...
    tuner = tune.Tuner(
        ...
        tune.with_parameters(
            objective_fn,
            params={'max_flips': 50000, 'max_runs': 1000, 'batch_size': 1, 'task': 'hpo'}
        ),
...
if __name__ == "__main__":
    ...
    tracking_uri = os.path.join(os.path.expanduser('~/'), 'projects/camsat/camsat_v2/data/experiments/tb/')
    print(tracking_uri) 
