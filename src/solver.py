...
def camsat(config: t.Dict, params: t.Dict) -> t.Union[t.Dict, t.Tuple]:
    ...
    for it in range(max_flips - 1):
        ...
        # at each iteration, noise is added to ram only for values of ram equal 1
        #fluctuations = cp.where(ram > 0, noise * cp.random.randn(*ram.shape, dtype=ram.dtype), 0)
        #y = matches @ (ram + fluctuations)
        y = matches @ ram
